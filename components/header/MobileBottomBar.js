...
/**
 * Открываем офф-канвас через глобальное событие (надёжно на iOS/Android/проде).
 * Никаких label/htmlFor и чекбоксов.
 */
...
export default function MobileBottomBar() {
  ...
  const openMenu = useCallback(() => {
    try {
      window.dispatchEvent(new CustomEvent("ui:drawer", { detail: { open: true } }));
      if (navigator?.vibrate) navigator.vibrate(10);
    } catch {}
  }, []);

  return (
    <div className="sd:hidden xz:block fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200">
      {/* 5 колонок: Главная / Меню / Профиль|Админ|Вход / Корзина / Позвонить */}
      <nav className="grid grid-cols-5 touch-manipulation select-none">
        {/* 1. Домой */}
        <Link
          href="/"
        ...
        <button
          ...
          className="flex flex-col items-center justify-center py-2 border-r border-gray-200 hover:bg-gray-50 cursor-pointer w-full"
        ...
        >
          {(dataApp?.dataKorzina?.length ?? 0) > 0 ? (
