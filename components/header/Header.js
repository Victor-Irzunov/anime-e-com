...
import TopBarFixedMobile from "./TopBarFixedMobile";
...
const Header = observer(() => {
  ...
  const [modalVisible, setModalVisible] = useState(false); // модалка остаётся для десктоп поиска
  ...
  return (
    <header className="fixed top-0 w-full z-50 overflow-x-hidden">
      {/* ==== MOBILE TOP (телефон + поиск) ==== */}
      <TopBarFixedMobile />

      {/* ==== DESKTOP HEADER ==== */}
      <div className="border-b border-b-gray-300 bg-white xz:hidden sd:block">
          ...
          <div className="navbar bg-base-100 py-0 layout-w">
            ...
            <div className="navbar-end gap-1">
              {user.userData?.isAdmin ? (
                <Link
                  href="/super-admin"
                  className="flex flex-col items-center text-gray-500 rounded-lg p-2 min-w-16 gap-1 hover:opacity-80 transition-opacity"
                >
              ...
              >
                <div className="indicator">
                  {dataApp.dataKorzina.length ? (
                    <span className="indicator-item px-1 w-4 h-4 text-[10px] font-semibold badge badge-secondary">
                      {dataApp.dataKorzina.length}
                    </span>
                  ) : null}
      ...
      {/* ==== DRAWER КАТЕГОРИЙ (мобилка). БЕЗ верхних иконок, просто чтобы burger работал через #my-drawer ==== */}
      <NavBarMobil
