...
const PICKUP_POINTS = [
    {
        id: "tolstogo",
        label:
            "Адрес Толстого 1 , 3 этаж большой островок ТЦ «Минск Сити Молл».",
    },
    {
        id: "dzerzh",
        label:
            "Адрес Дзержинского 106 , ТЦ «Магнит», 1 этаж , Магазин около бокового входа.",
    },
];
...
const saveShipping = (s) => {
    try {
        localStorage.setItem("shipping", JSON.stringify(s));
    } catch { }
};
...
function UserCart({ data, setData }) {
    ...
    const onSelectMethod = (method) => {
        if (method === "europost") {
            setAndSaveShipping({ method, format: "door", pickupPointId: null });
        } else if (method === "belpost") {
            setAndSaveShipping({ method, format: "pvz", pickupPointId: null });
        } else {
            setAndSaveShipping({ method, format: "-", pickupPointId: PICKUP_POINTS[0].id });
        }
    };
    ...
    const shippingHuman = useMemo(() => {
        if (shipping.method === "europost") {
            return {
                ...
                details:
                    "Стоимость доставки от 8р до 13р, точную стоимость назовет менеджер; при заказе от 100 рублей — доставка бесплатно",
            };
        }
        if (shipping.method === "belpost") {
            return {
                ...
                details:
                    "Стоимость доставки от 5р до 12р, точную стоимость назовет менеджер; при заказе от 100 рублей — доставка бесплатно",
            };
        }
        ...
        return {
            ...
            details: "Оплата онлайн или на месте. Забрать можно в часы работы торгового центра.",
        };
    }, [shipping]);
    ...
    return (
        <div className="my-2">
            ...
            <div className="flex sd:flex-row xz:flex-col gap-6 mb-10">
                {/* Левая колонка: список товаров + блоки доставки */}
                ...
                <div className="sd:w-80 xz:w-full flex flex-col gap-6">
                    ...
                    <div className="rounded-lg border border-gray-300 p-4 bg-white text-gray-700 flex flex-col gap-2">
                        ...
                        <div className="text-sm text-gray-500">Страна доставки:</div>
                        <div className="text-sm">Беларусь</div>

                        <div className="text-sm text-gray-500 mt-2">Способ доставки:</div>
                        <div className="text-sm">{shippingHuman.method}</div>

                        <div className="text-sm text-gray-500 mt-2">Формат доставки:</div>
                        <div className="text-sm">{shippingHuman.format}</div>

                        {shippingHuman.details ? (
                            <div className="text-xs text-gray-500 mt-2">{shippingHuman.details}</div>
                        ) : null}
            ...
            {isActive && user.isAuth ? (
                <OrderFormComp
                    ...
                    shipping={shipping} // передаем выбор доставки в форму
                />
            ) : null}

            {/* Подсказка авторизации */}
            {isActive && !user.isAuth ? (
                <div className="mb-32 mt-16">
                    <p className="text-lg">
                        <span className="font-semibold">Для оформления заказа вам необходимо</span>{" "}
                        <Link
                            href={{
