...
const ImgProductDetails = ({ product }) => {
  ...
  const handleNextImage = () => setSelectedImageIndex((i) => (i + 1) % imagesArr.length);

  return (
    <div className="bg-white p-6 rounded-lg border border-gray-300 flex sd:flex-row xz:flex-col gap-6">
      <div>
        <div className="sd:w-lg xz:w-auto aspect-4/3 overflow-hidden rounded-lg border border-gray-300">
          <div className="max-w-full cursor-pointer" onClick={() => setIsModalOpen(true)}>
            <img src={imageUrlAt(selectedImageIndex)} alt={product.title} className="w-full h-full object-cover" />
          </div>
        </div>
      ...
      <div className="flex-1">
        ...
        <div className="pb-3 pt-1 text-sm text-gray-400">На складе: {product.stock}</div>
        <div className="pb-3 pt-1 text-sm text-gray-400">Артикль: {product.id}</div>

        <div className="pt-1 flex items-center gap-3">
          <strong className="text-2xl font-medium text-gray-800">{product.price.toFixed(2)} руб</strong>
          <span className="font-medium text-gray-500 line-through">
            {(product.price / 100 * product.discountPercentage + product.price).toFixed(2)} руб
          </span>
        ...
        <div className="flex items-center gap-2">
          <div className="rating rating-sm disabled">
            {[1, 2, 3, 4, 5].map((r) => (
              <input key={r} type="radio" className={`mask mask-star-2 ${Math.round(product.rating) >= r ? "bg-orange-400" : "bg-gray-300"}`} readOnly />
            ))}
          </div>
        ...
        <div className="join py-4">
          <button onClick={handleDecrementQuantity} className="join-item btn btn-sm px-2 border border-gray-300">
            <RiSubtractFill fontSize={20} />
          </button>
          <input value={quantity} readOnly className="btn btn-sm px-4 join-item pointer-events-none bg-white border border-gray-300 w-14" />
          <button onClick={handleIncrementQuantity} className="join-item btn btn-sm px-2 border border-gray-300">
        ...
        <div className="modal-box">
          ...
          <div className="flex items-center justify-between">
            <div className="w-28 h-28 mt-5 mb-3 rounded-lg overflow-hidden border border-gray-300">
              <img src={imageUrlAt(0)} alt="" className="w-full h-full object-cover" />
            </div>
            <strong className="text-2xl font-medium text-gray-800">{product.price.toFixed(2)} руб</strong>
          </div>
          <div className="modal-action">
            <Link href="/korzina" className="btn">Перейти в корзину</Link>
            <form method="dialog"><button className="btn">Продолжить покупки</button></form>
          </div>
        </div>
      </dialog>
    </div>
